...
inline function t_stack_allocator::push(dword size, dword align) -> void * {
    let header_padding = m_distance_forward_to_align((u64) self.memory.ptr + self.head, alignof(dword));
    let data_padding = m_distance_forward_to_align((u64) self.memory.ptr + self.head + header_padding + sizeof(dword), align);
